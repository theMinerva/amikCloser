	...
	function talkTemplate(header,amikText,amikYear,amikWeek_,msgBox, currentBox, progressBar){
			var api = new mw.Api();
				...
				text: wikitext,minor: false, summary: "افزودن الگوی تاریخچهٔ آمیک به بحث مقالهٔ [["+header+"]] ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
				...
				text: wikitext,minor: false, summary: "افزودن الگوی تاریخچهٔ آمیک و ایجاد صفحهٔ بحث مقالهٔ [["+header+"]] ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
					}).done(function(result) {
	...
	function closeSuccess(header, msgBox, currentBox, amikYear, amikWeek_, progressBar){
		...
		api.get({action: 'parse',prop: 'wikitext', format: 'json', page:"ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/"+header}).done(function(data){
			...
			text: wikitext, minor: false, summary: "جمع‌بندی موفقانهٔ [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/"+header+"]] ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"}).done(function() {
	...
	//	}else if(currentRow.parentElement.tagName=='tbody'){var currentBox = currentRow;}
		...
		var archiveHardB = new OO.ui.ButtonWidget( {
						label: "بایگانی",
						title: "بایگانی",
						flags: [''], icon: "folderPlaceholder"
					});
		archiveHardB.on("click", function() {
			
			// A simple dialog window.
			function MyDialog( config ) {MyDialog.parent.call( this, config );}
			...
			this.pBox = new OO.ui.ComboBoxInputWidget( {
				options: pArr, label:":",
				labelPosition:"after",maxLength:3,value:pArr[0]['data'], dir:"rtl"
				} )
			...
			this.submitArch = new OO.ui.ButtonWidget( {
				label: "ارسال «"+header.slice(0,20)+"» به بایگانی انتخاب‌شده", flags: ['progressive']
					});
	...
	//			myDialog.$element.remove()
				...
				api.get({action:'parse',page:pagePref+toFa(archNum), prop:"wikitext",format:"json"
				}).done(function(data){
				...
				function archiveTransfer(preText){
					...
					api.get({action:'parse', page:'ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس', format:'json', prop:'wikitext'
					}).done(function(data){
					...
					}).fail(function(){myDialog.label1.setLabel("خطا در دریافت صفحهٔ وپ:پامیک از سرور.")})
			...
			var myDialog = new MyDialog( {size: 'medium',escapable:false} );
	...
	function toEn(foo){
		...
		if(document.title.slice(0,27) == 'ویکی‌پدیا:آیا می‌دانستید که' || document.title.slice(0,27) == 'کاربر:Nightdevil/صفحه تمرین'){
			...
			//get archives list
			var api=new mw.Api()
			api.get({action:'query',list:'allpages',apnamespace:4,apprefix:'آیا می‌دانستید که...؟/پیش‌نویس/بایگانی ', format:'json',aplimit:'max',apfilterredir:'nonredirects'
			}).done(function(data){
			...
			}).fail(function(){console.log("error getting archive data")})
			
			//get soft archives list
			api.get({action:'query',list:'allpages',apnamespace:4,apprefix:'آیا می‌دانستید که...؟/پیش‌نویس/جمع‌بندی‌های نرم/بایگانی ', format:'json',aplimit:'max',apfilterredir:'nonredirects'
			}).done(function(data){
			...
			}).fail(function(){console.log("error getting soft archives data")})
