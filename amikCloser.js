  ...
  function editFinalText(amikText, amikWeekURL, msgBox, header, amikYear, amikWeek_, currentBox, progressBar,
    amikTextOp) {
    var newAmikText;
    ...
    MyProcessDialog.prototype.getActionProcess = function (action) {
      ...
      if (action) {
        return new OO.ui.Process(function () {
          ...
          processSucessClose(amikText, amikWeekURL, msgBox, header, amikYear, amikWeek_, currentBox,
            progressBar)
          dialog.close({
            action: action
          });
  ...
  function processSucessClose(amikText, amikWeekURL, msgBox, header, amikYear, amikWeek_, currentBox, progressBar) {
    ...
    api.get({
      ...
      if (getWiki(data, msgBox, header).includes("… <!-- متن آمیک -->؟")) {
        ...
        var editSummary = "افزودن آمیک [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" + header +
          "]] به الگوی هفتگی ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
        ...
        api.postWithEditToken({
          ...
          msgBox.setLabel(
            "الگوی هفته با موفقیت ذخیره شد. در حال افزودن الگوی {{تاریخچه مقاله}} آمیک به بحث مقاله."
          )
        ...
        msgBox.setLabel("الگوی هفتهٔ انتخاب‌شده (" + amikWeekURL +
          ") جای خالی ندارد. هفته‌ای دیگر را انتخاب کنید.");
      ...
      api.get({
        ...
        api.postWithEditToken({
          ...
          text: wikitext,
          minor: false,
          summary: "افزودن آمیک [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" + header +
            "]] به الگوی هفتگی ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
        }).done(function (result) {
          msgBox.setLabel(
            "الگوی هفته با موفقیت ایجاد شد. در حال افزودن الگوی {{تاریخچه مقاله}} آمیک به بحث مقاله."
          )
          talkTemplate(header, amikText, amikYear, amikWeek_, msgBox, currentBox,
            progressBar)
        }).fail(function () {
          msgBox.setLabel("خطا در ذخیره کردن الگوی هفته. عملیات متوقف شد.")
        })
  ...
  function talkTemplate(header, amikText, amikYear, amikWeek_, msgBox, currentBox, progressBar) {
    ...
    api.get({
      ...
      var wikitext = getWiki(data, msgBox, header) + "{{تاریخچه مقاله| dykdate = " + amikYear.value +
        "0100+" + amikWeek_ + "weeks| dykentry = " + amikText +
        "| dyklink = ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" + header + "}}";
      api.postWithEditToken({
        action: 'edit',
        //title: "بحث:"+header,
        ...
        text: wikitext,
        minor: false,
        summary: "افزودن الگوی تاریخچهٔ آمیک به بحث مقالهٔ [[" + header +
          "]] ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
      }).done(function (result) {
        msgBox.setLabel(
          "الگوی تاریخچه با موفقیت به بحث مقاله افزوده شد. در حال افزودن الگوی {{بسته}} آمیک به این گفتگو."
        )
      ...
      var wikitext = "{{رتب}}{{بصب}}{{تاریخچه مقاله| dykdate = " + amikYear.value + "0100+" + amikWeek_ +
        "weeks| dykentry = " + amikText + "| dyklink = ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" +
        header + "}}";
      api.postWithEditToken({
        action: 'edit',
        //title: "بحث:"+header,
        ...
        text: wikitext,
        minor: false,
        summary: "افزودن الگوی تاریخچهٔ آمیک و ایجاد صفحهٔ بحث مقالهٔ [[" + header +
          "]] ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
      }).done(function (result) {
        msgBox.setLabel(
          "صفجه بحث مقاله ساخته شد و الگوی تاریخچه با موفقیت به آن افزوده شد. در حال افزودن الگوی {{بسته}} آمیک به این گفتگو."
        )
        closeSuccess(header, msgBox, currentBox, amikYear, amikWeek_, progressBar);
      }).fail(function (error) {
        msgBox.setLabel("خطا در ذخیره کردن الگوی هفته. عملیات متوقف شد.")
  ...
  function closeSuccess(header, msgBox, currentBox, amikYear, amikWeek_, progressBar) {
    ...
    api.get({
      ...
      var wikitext = getWiki(data, msgBox, header).replace(/(\=\=.+?=\=)/s, "$1\n{{بسته}}\n{{شد}} ~~" + "~~")
        .replace("| هفته =", "| هفته =" + amikWeek_).replace("| سال =", "| سال =" + amikYear.value) +
        "{{پایان بسته}}";
      api.postWithEditToken({
        action: 'edit',
        ...
        text: wikitext,
        minor: false,
        summary: "جمع‌بندی موفقانهٔ [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" + header +
          "]] ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
      }).done(function () {
        ...
        progressBar.$element.remove();
        currentBox.innerHTML += '<p>آمیک <b>' + header +
          '</b> به <a href="https://fa.wikipedia.org/wiki/ویکی‌پدیا:آیا می‌دانستید که...؟/' +
          amikYear.value + '/هفته ' + amikWeek_ + '">الگوی هفتهٔ ' + amikWeek_ + '</a> از سال ' +
          amikYear.value +
          ' افزوده شد.</p><p>تاریخچهٔ آمیک در <a href="https://fa.wikipedia.org/wiki/بحث:' +
          header + '">صفحهٔ بحث مقالهٔ ' + header +
          '</a> ثبت شد.</p><p>صفحهٔ <a href="https://fa.wikipedia.org/wiki/ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/' +
          header + '">گفتگوی پیش&zwnj;نویس آمیک</a> بسته شد.</p><div></div>';
  ...
  function closeFailSuccess(header, msgBox, currentBox, progressBar, jamReason) {
    ...
    currentBox.innerHTML +=
      '<p>صفحهٔ <a href="https://fa.wikipedia.org/wiki/ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/' + header +
  ...
  function addArchiveButtons_(currentBox, header, isSoft) {
    ...
    archiveHardB.on("click", function () {
      ...
      MyDialog.prototype.initialize = function () {
        ...
        this.submitArch.on("click", function () {
          ...
          if (myDialog.pBox.value == "") {
            ...
            api.get({
              ...
              myDialog.label1.setLabel(
                "بایگانی دریافت شد. در حال افزودن مبحث آمیک و ذخیره صفحه " +
                toFa(archNum))
              var preText = data.parse.wikitext['*'];
              archiveTransfer(preText)
            }).fail(function () {
              myDialog.label1.setLabel("این صفحه وجود ندارد. در حال ایجاد ‌‌ " +
                pagePref + toFa(archNum))
            ...
            function archiveTransfer(preText) {
              api.postWithEditToken({
                ...
                text: preText +
                  ...
                  "}}",
                summary: "بایگانی پیش‌نویس آمیک [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" +
                  header + "]] ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
              }).done(function () {
                myDialog.label1.setLabel(
                  "صفحه بایگانی ذخیره شد. در حال حذف زیرصفحه از صفحهٔ پیش‌نویس وپ:پامیک"
                )
                api.get({
                  action: 'parse',
                  ...
                  var newText = data.parse.wikitext['*'].replace(
                  ...
                  api.postWithEditToken({
                    ...
                    myDialog.label1.setLabel(
                      "صفحهٔ پیش‌نویس آمیک بایگانی شد. برای بستن این دیالوگ روی دکمهٔ ادامه کلیک کنید."
                    )
                    ...
                    if (currentBox.parentElement
                      .parentElement.tagName == "div") {
                      currentBox.parentElement
                        .parentElement.innerHTML =
                        "<div style='text-align: center; height: 100px; background-color: white; padding: 100px;'>آمیک " +
                        header +
                        " به بایگانی <a href='http://fa.wikipedia.org/wiki/ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/بایگانی " +
                        toFa(archNum) +
                        "'>ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/بایگانی " +
                        toFa(archNum) +
                        "</a> منتقل شد.</div>"
                    } else {
                      currentBox.parentElement
                        ...
                        "<div style='text-align: center; height: 100px; background-color: white; padding: 100px;'>آمیک " +
                        header +
                        " به بایگانی <a href='http://fa.wikipedia.org/wiki/" +
                        pagePref + toFa(archNum) +
                        "'>" + pagePref + toFa(
                          archNum) +
                        "</a> منتقل شد.</div>"
                    ...
                    myDialog.$body.append(this.clDiag
                      .$element);

                  }).fail(function () {
                    myDialog.label1.setLabel(
                      "خطا در ذخیره وپ:پامیک.")
                  })
                }).fail(function () {
                  myDialog.label1.setLabel(
                    "خطا در دریافت صفحهٔ وپ:پامیک از سرور.")
                })
        ...
        this.stackLayout.$element[0].children[1].style =
          "margin:69px;margin-top: 100px; text-align:center";
  ...
  function getWiki(data, msgBox, header) {
    try {
      ...
      msgBox.setLabel("خطا در خواندن [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" + header +
        "]]. ممکن است عنوان آمیک پیشنهادی در صفحه پیش‌نویس اشتباه وارد شده باشد و باید دستی جمع‌بندی شود.")
    }
  }
        ...
        if (confirmed) {
          ...
          if (jamReason.value == "جمع‌بندی نرم") {
            ...
            api.get({
              ...
              wikiText = data.parse.wikitext['*'].replace(/(\=\=.+?=\=)/s,
                "$1\n<small> </small>\n") + "{{جا:جمع‌بندی نرم پامیک}}";
              msgBox.setLabel(
                'الگوی جمع‌بندی نرم پامیک تراگنجانیده شد. در حال ذخیرهٔ صفحهٔ پیش‌نویس ' +
                header)
              api.postWithEditToken({
                action: "edit",
                //title:"ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/"+header,
                ...
                text: wikiText,
                summary: "جمع‌بندی ناموفق [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" +
                  header + "]] (" + jamReason.value +
                  ") ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
              }).done(function () {
                closeFailSuccess(header, msgBox, currentBox, progressBar, jamReason)
            ...
            //hard close fail
            $.ajax({
              url: "https://fa.wikipedia.org/w/api.php?action=parse&page=ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" +
                header + "&prop=wikitext&format=json",
              success: function (data) {
                var wikitext = getWiki(data, msgBox, header).replace(/(\=\=.+?=\=)/s,
                    "$1\n{{بسته|ناموفق=بله}}\n" + jamReason.value + "  ~~" + "~~") +
                  "{{پایان بسته}}"
                //console.log(q2)
                var editSummary =
                  "جمع‌بندی ناموفق [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" +
                  header + "]] (" + jamReason.value +
                  ") ([[وپ:اجآ|ابزار جمع‌بندی آمیک]])"
                ...
                postEdit_(wikitext, editSummary, pageName, header, msgBox, currentBox,
                  progressBar, jamReason);
              },
  ...
  function addButtons_(currentRow, header) {
    ...
    //نوار قالبش
    $(currentRow).append(
      '<tr><td colspan="2"><div style="display: flex; justify-content: center;"></div></td></tr><tr style="display:none;"><td colspan=2 style="padding-bottom:10px; text-align:center"><div style="display: flex; justify-content: center;"></div></td></tr>'
    );
    ...
    function week(year, month, day) {
      ...
      var date = year instanceof Date ? year.valueOf() : typeof year === "string" ? new Date(year).valueOf() :
    ...
    doneB.on("click", function () {
      OO.ui.confirm("آیا از جمع‌بندی موفقانهٔ این پیشنهاد (" + header + ") اطمینان دارید؟").done(function (
        confirmed) {
        if (confirmed) {
          //msg box
          var msgBox = new OO.ui.MessageWidget({
          ...
          $(currentBox).append(msgBox.$element);
          //
          var amikWeek_ = amikWeek.value.replace(/1/g, "۱").replace(/2/g, "۲").replace(/3/g, "۳")
            .replace(/4/g, "۴").replace(/5/g, "۵").replace(/6/g, "۶").replace(/7/g, "۷")
            .replace(/8/g, "۸").replace(/9/g, "۹").replace(/0/g, "۰")
          var amikWeekURL = "ویکی‌پدیا:آیا می‌دانستید که...؟/" + amikYear.value + "/هفته " +
            amikWeek_
          //get amikText
          var api = new mw.Api();
          api.get({
            ...
            var amikText = getWiki(data, msgBox, header).match(/\{\{گفتاورد\|(.*?)؟/)[
              0].substring(10)
            var amikTextOp = getWiki(data, msgBox, header).match(
              /\{\{گفتاورد\|(.*?)؟/g)
            editFinalText(amikText, amikWeekURL, msgBox, header, amikYear, amikWeek_,
              currentBox, progressBar, amikTextOp)

          }).fail(function (error) {
            msgBox.setLabel(
              "خطا در خواندن [[ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/" +
              header +
              "]]. ممکن است عنوان آمیک پیشنهادی در صفحه پیش‌نویس اشتباه وارد شده باشد و باید دستی جمع‌بندی شود."
            )
            console.log("failed")
    ...
    var topLayout = new OO.ui.FieldsetLayout({
      helpInline: true,
      help: "مشخص کنید که آمیک در چه هفته از چه سالی در صفحهٔ اصلی نمایش یابد. الان در هفتهٔ " + toFa(week(
          currentTime.getFullYear(), currentTime.getMonth() + 1, currentTime.getDate())) + "م از سال " +
        toFa(currentTime.getFullYear()) + " هستیم"
    });
  ...
  function toFa(foo) {
    return foo.toString().replace(/1/g, "۱").replace(/2/g, "۲").replace(/3/g, "۳").replace(/4/g, "۴").replace(/5/g,
      "۵").replace(/6/g, "۶").replace(/7/g, "۷").replace(/8/g, "۸").replace(/9/g, "۹").replace(/0/g, "۰")
  }

  function toEn(foo) {
    return foo.toString().replace(/۱/g, 1).replace(/۲/g, 2).replace(/۳/g, 3).replace(/۴/g, 4).replace(/۵/g, 5).replace(
      /۶/g, 6).replace(/۷/g, 7).replace(/۸/g, 8).replace(/۹/g, 9).replace(/۰/g, 0)
  }

  if (document.title.slice(0, 27) == 'ویکی‌پدیا:آیا می‌دانستید که' || document.title.slice(0, 27) ==
    'کاربر:Nightdevil/صفحه تمرین') {
    var amikBoxes = document.getElementsByClassName("infobox")
    if (amikBoxes.length != 0) {
      mw.loader.using(["oojs-ui-core", "oojs-ui-widgets", "oojs-ui-windows"]).done(function () {
        mw.loader.load(["oojs-ui.styles.icons-alerts", "oojs-ui.styles.icons-interactions",
          "oojs-ui.styles.icons-moderation", "oojs-ui.styles.icons-user",
          "oojs-ui.styles.icons-content", "oojs-ui.styles.icons-editing-core",
          "oojs-ui.styles.icons-editing-advanced"
        ]);
        for (var i = 0; i < amikBoxes.length; i++) {
          var currentBox = amikBoxes[i].children[0]; //تعریف فضای فعلی
          var header = amikBoxes[i].getElementsByClassName("infobox-header")[0].textContent.replace(
            'بررسی آمیک ', ''); //گرفتن عنوان
          ...
          if (amikBoxes[i].parentElement.children[0].tagName ==
            "SMALL") { //چک کردن اینکه قبلا بسته شده است (hard)
            var isSoft = false;
            addArchiveButtons_(currentBox, header, isSoft);
          } else if (amikBoxes[i].children[0].children[0].children[0].textContent.slice(6, 10) ==
            "آمیک") {
    ...
    api.get({
      ...
      for (i = 0; i < archs.length; i++) {
        archsTemp[i]['title'] = parseInt(toEn(archsTemp[i]['title'].replace(
          'ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/بایگانی ', "")), 10)
    ...
    api.get({
      ...
      for (i = 0; i < archs.length; i++) {
        archsSofts[i]['title'] = parseInt(toEn(archsSofts[i]['title'].replace(
          'ویکی‌پدیا:آیا می‌دانستید که...؟/پیش‌نویس/جمع‌بندی‌های نرم/بایگانی ', "")), 10)
